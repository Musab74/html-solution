...
try{
	...
	for($i = 2; $i <= $maxRow; $i++) {
		...
		$ContentsPage = $objWorksheet->getCell('G' . $i)->getValue(); // 차시페이지수
		$ContentsMobilePage = $objWorksheet->getCell('H' . $i)->getValue(); // 모바일페이지수
		$ContentsURL = $objWorksheet->getCell('I' . $i)->getValue(); // 콘텐츠 경로
		$MobileURL = $objWorksheet->getCell('J' . $i)->getValue(); // 모바일 경로
		...
		*/
		...
		$Sql = "INSERT INTO ContentsExcelTemp 
				(idx, Gubun, ContentsTitle, LectureTime, Expl01, Expl02, Expl03, ContentsType, ContentsPage , ContentsMobilePage, ContentsURL, MobileURL, ID, RegDate) 
				VALUES ($maxno, '$Gubun', '$ContentsTitle', $LectureTime, '$Expl01', '$Expl02', '$Expl03', 'A', '$ContentsPage', '$ContentsMobilePage', '$ContentsURL', '$MobileURL', '$LoginAdminID', NOW())";
		//echo $k.":".$Sql."<BR><BR>";
