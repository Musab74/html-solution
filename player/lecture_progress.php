...
// 부모가 보내는 현재 위치(초) — A/B 공통

$PositionSec = Replace_Check_XSS2($PositionSec);
$SeekFrom = (int)Replace_Check_XSS2($_POST['SeekFrom'] ?? $_GET['SeekFrom'] ?? 0);
$SeekTo   = (int)Replace_Check_XSS2($_POST['SeekTo']   ?? $_GET['SeekTo']   ?? 0);
$DeltaSec = $_POST['DeltaSec'] ?? $_GET['DeltaSec'] ?? 0;
...
if ($ProgressStep === 'Seek') { $TriggerYN = "N"; }
...
//수강현황 로그 작성
$is_seek = ($ProgressStep === 'Seek') ? 1 : 0;
$pos_sec = ($PositionSec > 0) ? $PositionSec : null;

$sf = ($SeekFrom > 0 ? $SeekFrom : null);
$st = ($SeekTo   > 0 ? $SeekTo   : null);

$Sql_log = "INSERT INTO ProgressLog(
  ID, LectureCode, Study_Seq, Chapter_Seq, Contents_idx, ContentsDetail_Seq,
  LastStudy, PositionSec, IsSeek, SeekFromSec, SeekToSec, DeltaSec,
  Progress, StudyTime, UserIP, RegDate, TriggerYN, Chapter_Number, TotalProgress
) VALUES (
  '$LoginMemberID', '$LectureCode', $Study_Seq, $Chapter_Seq, $Contents_idx, $ContentsDetail_Seq,
  '$LastStudy', ".($pos_sec===null?'NULL':$pos_sec).", $is_seek, ".($sf===null?'NULL':$sf).", ".($st===null?'NULL':$st).", ".($DeltaSec===null?'NULL':$DeltaSec).",
