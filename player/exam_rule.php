...
if($Row) {
	...
	if($TestType=="MidTest")   $ExamStatus = $MidStatus;
	if($TestType=="Test")      $ExamStatus = $TestStatus;
	if($TestType=="Report")    $ExamStatus = $ReportStatus;
...
<!-- info -->
	<div class="ruleArea">
		...
		<p class="tc"><img src="img/eduexam_rule_title.png" alt="주의사항" /></p>
		<ol>
			<?if($TestType=="MidTest") {?>
			<li>중간평가의 <span class="fcRed01B">응시 제한 시간은 <?=$TestTime?>분</span>입니다.</li>
			<li>중간평가는 <span class="fcRed01B">훈련기간 종료일 다음날로부터 1주일 이내 재응시(N차 평가) 실시 가능</span>합니다.<br />기존 원격훈련 중 국민내일배움카드에서만 N차 평가를 허용하였으나, 관련 규정 개정(‘21.1.1.)으로 사업주훈련에서도 동일기간 내에서 N차 평가 실시 가능</li>
			<li><span class="fcRed01B">반드시 PC에서 진행</span>하여 주시고, 장애가 발생하지 않도록 유선 인터넷 등을 사용하시기 바랍니다.</li>
			<li><span class="fcRed01B">최종제출 버튼까지 클릭</span> 후 제출해야지만 평가응시가 완료됩니다.</li>
			<li>최종제출을 하지 않은 상태로 평가시간이 종료될 경우 <span class="fcRed01B">작성한 답안으로 자동 제출</span>됩니다.</li>
			<li>중간평가의 점수는 최종 수료 점수에 반영됩니다. <span class="fcRed01B">수료기준의 반영률을 확인</span>하여 주십시오.</li>
			<li>평가 결과는 학습기간이 종료되고, <span class="fcRed01B">첨삭이 완료된 후 제공</span>됩니다.</li>
			<li>평가가 진행되는 도중 <span class="fcRed01B">평가창을 닫거나 PC가 꺼질 경우에는 평가시간은 계속 진행</span>되고,<br />재접속 여부와 상관없이 <span class="fcRed01B">기존 평가 시간이 모두 소진되면 자동으로 평가가 종료</span>됩니다.</li>
			<li>평가 진행 도중 입력 및 선택한 답안은 임시저장 기능으로 저장되며, 창을 닫았다 다시 열어도 저장된 내용이 남아있습니다.</li>
			<?}?>
			<?if($TestType=="Test") {?>
			<li>최종평가의 <span class="fcRed01B">응시 제한시간은 <?=$TestTime?>분 입니다.</span></li>
			<li>최종평가는 <span class="fcRed01B">훈련기간 종료일 다음날로부터 1주일 이내 재응시(N차 평가) 실시 가능</span>합니다.<br />기존 원격훈련 중 국민내일배움카드에서만 N차 평가를 허용하였으나, 관련 규정 개정(‘21.1.1.)으로 사업주훈련에서도 동일기간 내에서 N차 평가 실시 가능</li>
			<li><span class="fcRed01B">반드시 PC에서 진행</span>하여 주시고, 장애가 발생하지 않도록 유선 인터넷 등을 사용하시기 바랍니다.</li>
			<li><span class="fcRed01B">최종제출 버튼까지 클릭</span> 후 제출해야지만 평가 응시가 완료됩니다.</li>
			<li>최종제출을 하지 않은 상태로 평가시간이 종료될 경우 <span class="fcRed01B">작성한 답안으로 자동 제출</span>됩니다.</li>
			<li>최종평가의 점수는 최종 수료 점수에 반영됩니다. <span class="fcRed01B">수료기준의 반영률을 확인</span>하여 주십시오.</li>
			<li>평가 결과는 학습기간이 종료되고, <span class="fcRed01B">첨삭이 완료된 후 제공</span>됩니다. </li>
			<li>평가가 진행되는 도중 <span class="fcRed01B">평가창을 닫거나 PC가 꺼질 경우에도 평가시간은 계속 진행</span>되고<br>재접속여부와 상관없이 <span class="fcRed01B">기존 평가 시간이 모두 소모되면 자동으로 평가가 종료</span>됩니다. </li>
			<li>평가 진행 도중 입력 및 선택한 답안은 임시저장 기능으로 저장되며, 창을 닫았다 다시 열어도 저장된 내용이 남아있습니다.</li>
			<li><span class="fcRed01B"><모사답안의 정의></span><br>
			· 훈련생간에 제출답안 비교시 100% 동일 답안으로 판별될 경우(제출한 답안의 byte수 검사)<br>
			· 서술한 답안의 유사성이 확실한 경우<br>
			· 오타 및 띄어쓰기, 접두/접미어를 이용한 유사한 답안을 제출한 경우<br>
			· 모사답안 판정 여부는 첨삭강사에 의해 최종 결정
			</li>
			<li><span class="fcRed01B"><모사답안 발생 시 처리기준></span><br>
			...
			<?}?>
			<?if($TestType=="Report") {?>
			<li>과제의 <span class="fcRed01B">응시 제한시간은 <?=$TestTime?>분 입니다.</span></li>
			<li>과제는 <span class="fcRed01B">훈련기간 종료일 다음날로부터 1주일 이내 재응시(N차 평가) 실시 가능</span>합니다.<br />기존 원격훈련 중 국민내일배움카드에서만 N차 평가를 허용하였으나, 관련 규정 개정(‘21.1.1.)으로 사업주훈련에서도 동일기간 내에서 N차 평가 실시 가능</li>
			<li><span class="fcRed01B">반드시 PC에서 진행</span>하여 주시고, 장애가 발생하지 않도록 유선 인터넷 등을 사용하시기 바랍니다.</li>
			<li><span class="fcRed01B">최종제출 버튼까지 클릭</span> 후 제출해야지만 평가 응시가 완료됩니다.</li>
			<li>최종제출을 하지 않은 상태로 평가시간이 종료될 경우 <span class="fcRed01B">작성한 답안으로 자동 제출</span>됩니다.</li>
			<li>과제의 점수는 최종 수료 점수에 반영됩니다. <span class="fcOrg01B">수료기준의 반영률을 확인</span>하여 주십시오.</li>
			<li>평가 결과는 학습기간이 종료되고, <span class="fcOrg01B">첨삭이 완료된 후 제공</span>됩니다. </li>
			<li>평가가 진행되는 도중 <span class="fcOrg01B">평가창을 닫거나 PC가 꺼질 경우에도 평가시간은 계속 진행</span>되고<br>재접속여부와 상관없이 <span class="fcOrg01B">기존 평가 시간이 모두 소모되면 자동으로 평가가 종료</span>됩니다. </li>
			<li>평가 진행 도중 입력 및 선택한 답안은 임시저장 기능으로 저장되며, 창을 닫았다 다시 열어도 저장된 내용이 남아있습니다</li>
...
<script type="text/javascript">
    ...
    <?if($ServiceType==1 || $ServiceType==4) { //환급과정의 경우 ?>
        ...
        window.open('/player/motp.php?class_tme=<?=$Chapter_Number?>&COURSE_AGENT_PK=<?=$LectureCode?>&CLASS_AGENT_PK=<?=$LectureCode?>,<?=$LectureTerme_idx?>&type=<?=$TestType?>&Study_Seq=<?=$Study_Seq?>&Chapter_Seq=<?=$Chapter_Seq?>', 'popupMotp', 'width=552, height=962, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
    <?}?>

    $(document).keydown(function(e) {
		if(e.which===112 || e.which===114 || e.which===115 || e.which===116 || e.which===118 || e.which===122 || e.which===123 || e.which===17) {
			if(typeof event=="object")	event.keyCode = 0;
			return false;
		}
		if(e.which===82 && e.ctrlKey)	return false;
		if(e.which===78 && e.ctrlKey)	return false;
		if(e.which===67 && e.ctrlKey)	return false;
		if(e.which===86 && e.ctrlKey)	return false;
		if(e.which===220 && e.shiftKey) return false;
	});
