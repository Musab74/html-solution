...
function hmsToSec(hms){
	if(!hms) return 0;
	var s = String(hms).replace(/[^\d:]/g,'').split(':').map(Number);
...
function StudyProgressCheckWork(ProgressStep, CloseYN, ContentsURLSelect) {
	var LastStudy = '';
  
	if ($('#MultiContentType').val() == 'N') {
	  if ($('#ContentsType').val() == 'A') {
		// ★ A: 경로만 LastStudy에 저장(그대로 유지)
		var page = document.getElementById('mPlayer').contentWindow.location.pathname;
		LastStudy = page.replace('/contents', '');
	  }
	  if ($('#ContentsType').val() == 'B') {
	...
	var PositionSec = 0;
	if ($('#ContentsType').val() === 'A') {
	  var iframeTime = $('#mPlayer').contents().find('.time1 .playerText').text(); // H:MM:SS 텍스트
	  PositionSec = hmsToSec(iframeTime);
	} else if ($('#ContentsType').val() === 'B') {
	  PositionSec = Math.floor(mPlayer.currentTime || 0);
	}
	...
	$.ajax({
	  ...
	  data: {
		...
		ContentsDetail_Seq: ContentsDetail_Seq,
		ProgressTime: ProgressTime,
		LastStudy: LastStudy,         // 경로(또는 B일 때 초)
		CompleteTime: CompleteTime,
		ProgressStep: ProgressStep,
		PositionSec: PositionSec      // ★ 이어듣기용 “초”는 이것만 신뢰
	  }
	}).done(function (parseData) {
	  if (CloseYN === 'Y') location.reload();
	}).fail(function (xhr) {
	  console.error('lecture_progress.php failed:', xhr.status, xhr.responseText);
	});
  }
...
//수강후기 팝업
function SurveyPop(LectureCode, ChkData) {
    ScrollPosition = 100;
	left_position = 50;


	$('#StudyInformation').load('/player/survey_pop.php', { LectureCode: LectureCode, ChkData: ChkData}, function () {
		$("div[id='StudyInformation']")
			.css({
				top: ScrollPosition,
				left: left_position,
