...
$UPLOAD_DIR = "/web/hrdeArchive/html/upload";
$HomeDirectory = "/web/hrdeArchive/html";

$SiteURL 			= $Protocol_SSL."archive.hrdetest.com";
$MobileSiteURL 		= $Protocol_SSL."m.hrdetest.com";

$MobileAuthURL 		= $Protocol_SSL."archive.hrdetest.com/"; //모바일 인증 도메인
$FlashServerURL 	= $Protocol_SSL."archive.hrdetest.com/contents";
$MovieServerURL		= $Protocol_SSL."archive.hrdetest.com/contents";
$MobileServerURL 	= $Protocol_SSL."archive.hrdetest.com/contents";
...
$CheckPlus_sitecode = "";
$CheckPlus_sitepasswd = "";

$IPIN_CheckPlus_sitecode = "";
$IPIN_CheckPlus_sitepasswd = "";
...
function aligo_mms_send($phone, $subject, $emtitle, $msg, $callback, $template_code="hrd01") {
    ...
    if ($phone)  {
        ...
        $_port      =	(strtolower($_hostInfo['scheme']) == 'https') ? 443 : 80;
        $_variables =	array(
            'apikey'      => $apikey, //인증용 API Key
            'userid'      => $aligoUserid, //사용자 ID
            'token'       => $retArr->token,
            'senderkey'   => $aligoSenderkey, //발신프로파일 키
            'tpl_code'    => $template_code, // 템플릿 코드
            'sender'      => $callback, //발신자 연락처
            // 'senddate'    => date("YmdHis", strtotime("+10 minutes")), //예약일
            'receiver_1'  => $phone, //수신자 연락처
            'subject_1'   => $subject, //알림톡 제목
            //'emtitle_1'   => $emtitle, //강조표기형의 타이틀
            'message_1'   => $msg // 알림톡 내용
        );
