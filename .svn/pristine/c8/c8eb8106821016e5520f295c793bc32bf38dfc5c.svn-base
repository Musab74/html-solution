...
//cell border
$objPHPExcel->getActiveSheet()->getStyle('A1:AC'.$TOT_NO2)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
//align
$objPHPExcel->getActiveSheet()->getStyle('A1:AC'.$TOT_NO2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:AC'.$TOT_NO2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->getActiveSheet()->getStyle('A1:AC'.$TOT_NO2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);


//1행 처리
$objPHPExcel->getActiveSheet()->getStyle('A1:AC1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('E8E8E8E8');
...
$objPHPExcel->getActiveSheet()->getCell('U1')->setValue("기업관리코드");
...
$objPHPExcel->getActiveSheet()->getCell('AB1')->setValue("영업담당자 소속");
$objPHPExcel->getActiveSheet()->getCell('AC1')->setValue("메모");


$i=2;
$k = 1;
$SQL = "SELECT a.*, b.Name AS SalesName, b.Team AS SalesTeam FROM Company AS a LEFT OUTER JOIN StaffInfo AS b ON a.SalesManager=b.ID $where $orderby";
...
if($QUERY && mysqli_num_rows($QUERY)){
    while($ROW = mysqli_fetch_array($QUERY)){
    	...
    	$objPHPExcel->getActiveSheet()->getCell('AB'.$i)->setValueExplicit($SalesTeam, PHPExcel_Cell_DataType::TYPE_STRING);
    	$objPHPExcel->getActiveSheet()->getCell('AC'.$i)->setValueExplicit($Remark, PHPExcel_Cell_DataType::TYPE_STRING);
